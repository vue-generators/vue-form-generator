<template lang="pug">
	.form-group(:class='getFieldRowClasses(field)')
		...
		.field-wrap
			...
			button(v-for='(btn, index) in field.buttons', @click='buttonClickHandler(btn, field, $event)', :class='btn.classes', :key='index', v-text='btn.label', :type='getButtonType(btn)')
		.hint(v-if='field.hint', v-html='fieldHint(field)')
		.errors.help-block(v-if='fieldErrors(field).length > 0')
			span(v-for='(error, index) in fieldErrors(field)', :key='index', v-html='error')
</template>
